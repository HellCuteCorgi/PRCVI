...
class Ui_FuncWind(object):
    ...
    def UpdateTable(self, queryStr="select 'no', 'data', 'found'", conn=None):
        if (conn):
            ...
            self.AddNoteTable.setColumnCount(5)
            self.AddNoteTable.setHorizontalHeaderLabels(
                ['Файл разметки', 'Оригинальный файл', 'Тип', 'Регион', 'Время съёмки'])
            for row in cur:
                ...
                self.AddNoteTable.setItem(rows, 0, QTableWidgetItem(row[1]))
                self.AddNoteTable.setItem(rows, 1, QTableWidgetItem(row[2]))
                self.AddNoteTable.setItem(rows, 2, QTableWidgetItem(row[3]))
                self.AddNoteTable.setItem(rows, 3, QTableWidgetItem(row[4]))
                self.AddNoteTable.setItem(rows, 4, QTableWidgetItem(str(row[5])))
